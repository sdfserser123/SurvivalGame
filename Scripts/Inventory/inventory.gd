		...
		if event.button_index == MOUSE_BUTTON_LEFT && event.pressed:
			if holding_item != null:
				if !slot.item:
					...
					if holding_item.idName != slot.item.idName:
...
func left_click_same_item(slot: slotClass):
	...
	if able_to_add == 0:
		...
		var to_add = min(able_to_add, holding_item.item_amount)
		PlayerInventory.add_item_quantity(slot, to_add)
		slot.item.add_item_quantity(to_add)
		holding_item.decrease_item_quantity(to_add)

		if holding_item.item_amount <= 0:
